...
from lib.redis_helper import RedisShelf, redis_db
# from . import api
...
class EventsState(State):
    ...
    def event(self, event):
        ...
        redis_db.set('celery_events:{}'.format(event.get('uuid')), self.dump_json(task.as_dict()), expire=86400 * 30)
        logger.info(f"{worker_name} {task.name} {task_id} {event_type}")
    ...
    def task_info(self, task_id):
        json_info = redis_db.get('celery_events:{}'.format(task_id))
        dic_info = json.loads(json_info)
        return dic_info

    def itertasks(self, cursor=0, limit=None):
        _, tasks = redis_db.scan(cursor=cursor, match="celery_events:*", count=limit)
        index = 0
        for k in tasks:
            yield json.loads(redis_db.get(k))
...
class Events(threading.Thread):
    ...
    def __init__(self, capp, db=None, persistent=False,
                 enable_events=True, io_loop=None, key_prefix=None, **kwargs):
        ...
        if self.persistent:
            ...
            # state = RedisShelf(redis=redis_db.conn, key_prefix=self.key_prefix)
            # self.state = state.get('events')
            # state.close()
    ...
    def stop(self):
        if self.persistent:
            logger.debug("Saving state to redis '%s'...", self.db)
            # state = RedisShelf(redis=redis_db2.conn, key_prefix=self.key_prefix)
