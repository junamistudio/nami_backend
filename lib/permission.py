"""
    Example:
    >>> from lib.permission import UserNeed, RoleNeed, ItemNeed, Permission
...
__all__ = ['UserNeed', 'DeptNeed', 'RoleNeed', 'ItemNeed', 'Permission', 'Identity', 'AnonymousIdentity',
           'PermissionDenied']
...
DeptNeed = partial(Need, 'dept')
DeptNeed.__doc__ = """A need with the method preset to `"dept"`."""
...
class Identity(object):
    ...
    def __repr__(self):
        return '<{0} id="{1}" provides={2}>'.format(
            self.__class__.__name__, self.name, pprint.pformat(self.provides)
        )
...
class IdentityContext(object):
    ...
    def __enter__(self):
        ...
        if not self.can():
            logger.info((self.permission, self.identity))
